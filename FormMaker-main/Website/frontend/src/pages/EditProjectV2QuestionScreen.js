...
import { getProjectcDetails, listMyProjects, updateProjectV2Questions } from '../actions/projectActions';
import { PROJECT_UPDATE_QUESTION_RESET } from '../constants/projectConstants';

const EditProjectV2QuestionScreen = () => {
  ...
  useEffect(() => {
    if (successUpdate) {
      ...
      setResultAndObservation(data1.version2[index].questions.resultAndObservation);
      setRecomendation(data1.version2[index].questions.recomendation);
      setAuditorRating(data1.version2[index].questions.auditorRating);
    } else {
      navigate('/404');
    }
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    console.log('data');
    dispatch(updateProjectV2Questions(index, resultAndObservation, auditorRating, recomendation, projectId, proof));
    //  navigate(`/project/${projectId}/open`);
  };

  // .question); // .map((que) => console.log(que.questions.question));
  // project.version2.forEach((element, i) => {
          ...
          <Box sx={style}>
            <TableContainer sx={{ maxHeight: 540 }}>
              <Typography component="h1" variant="h4">
                Question :{data1.version2[index].questions.question}
                {/* {data1.version2[index].questions.question} ? */}
              </Typography>
              <Divider />
              <Typography component="h1" variant="h5">
                Area Of Audit : {data1.version2[index].questions.areaofaudit}.
              </Typography>

              <Typography component="h1" variant="body2">
                <span style={{ fontWeight: 800, fontSize: 16 }}>Description :</span>{' '}
                {data1.version2[index].questions.description}.
              </Typography>
              <Divider />
              <Typography component="h1" variant="body2">
                <span style={{ fontWeight: 800, fontSize: 16 }}>Expected Prooof : </span>
                {data1.version2[index].questions.expectedProofs}.
              </Typography>

              <Typography component="h1" variant="h5">
                Threat : {data1.version2[index].questions.threat}.
              </Typography>
                ...
                <Grid container sx={{ marginTop: 4 }}>
                  <Grid item xs={20}>
                    <TextField
                      ...
                      multiline
                      size="small"
                      //  defaultValue={data1.version2[index].questions.resultAndObservation}
                  ...
                  <Grid item xs={20} sx={{ marginTop: 4 }}>
                    <TextField
                      ...
                      size="small"
                      value={recomendation}
                      //  defaultValue={data1.version2[index].questions.recomendation}
...
export default EditProjectV2QuestionScreen;
