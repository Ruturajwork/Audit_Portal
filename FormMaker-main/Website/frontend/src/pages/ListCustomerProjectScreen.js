...
import { listMyCProjects } from '../actions/projectActions';
...
export default function ListCustomerProjectScreen() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = projects.map((n) => n.fname);
            ...
            <TableCell align="center">Delete</TableCell> */}
                ...
                <TableBody>
                  {projects.map((projec, index) => (
                    <TableRow key={projec._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      ...
                      <TableCell align="center" component={RouterLink} to={`/project/cust/${projec._id}/open`}>
                ...
                <IconButton variant="outlined" component={RouterLink} to={`/project/${projec._id}/edit`}>
                  <Icon as={ModeEditIcon} sx={{ color: green[500] }} w="8" h="8" />
                      ...
                      {/* button for delete  */}
                      {/* <TableCell>
                <IconButton aria-label="delete" size="large" color="error" onClick={() => deleteHandler(projec._id)}>
